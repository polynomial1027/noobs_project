...
from snake_ai.src.utils.config import CNNDQNConfig, SnakeConfig
...
def train_cnn_dqn(
    ...
    print(f"Grid shape: channels={dqn_config.input_channels}, rows={env.rows}, cols={env.cols}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train CNN-DQN agent on Snake.")
    parser.add_argument(
        "--episodes",
        type=int,
        default=CNNDQNConfig.max_episodes,
        help="Number of training episodes.",
    )
    parser.add_argument(
        "--render-every",
        type=int,
        default=CNNDQNConfig.render_every,
        help="Render one episode every N episodes. Use 0 to disable rendering.",
    )
    parser.add_argument(
        "--save-every",
        type=int,
        default=CNNDQNConfig.save_every,
        help="Save checkpoint every N episodes. Use 0 to disable periodic saving.",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="snake_ai/checkpoints/cnn_dqn_latest.pth",
